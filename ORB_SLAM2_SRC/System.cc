...
/**
* Affordable SmartWheelchair Project: Updated ORB-SLAM2
* Implemented new features such as "placeRecognition" and "MarkLocation"
* MarkLocation marks the locaiton and then stores the keyframes in a txt file
* placeRecognition handles localization and determines whether we are at
* marked location by comparing current keyframe to the txt file.
**/
...
void System::SaveTrajectoryKITTI(const string &filename)
{
    ...
    cin >> location;
    cout << "location: " << location << " saved!" << endl;
...
string System::keyFrameTimeStamp_sim3(const string &sim3File){
	string location;
	cout << "please enter location name" << endl;
	cin >> location;
...
void System::saveKeyFrame(const string &currentFrame){
	vector<KeyFrame*> vpKFs = mpMap->GetAllKeyFrames();
	sort(vpKFs.begin(),vpKFs.end(),KeyFrame::lId);

	ofstream f1;
	f1.open(currentFrame.c_str());
	f1 << fixed;
	...
	cv::Mat pose = pKF->GetPose();
      ...
      f1 << setprecision(8) << pose.at<float>(0,0) << " " << pose.at<float>(0,1)  << " " << pose.at<float>(0,2) << " "  << pose.at<float>(0,3) << endl <<
                               pose.at<float>(1,0) << " " << pose.at<float>(1,1)  << " " << pose.at<float>(1,2) << " "  << pose.at<float>(1,3) << endl <<
                               pose.at<float>(2,0) << " " << pose.at<float>(2,1)  << " " << pose.at<float>(2,2) << " "  << pose.at<float>(2,3) << endl <<
                               pose.at<float>(3,0) << " " << pose.at<float>(3,1)  << " " << pose.at<float>(3,2) << " "  << pose.at<float>(3,3) << endl;
}

bool System::placeRecognition(){
	bool locationRecognized = true;
	int badData = 0;
	double threshhold = 0.1, diff;

	ifstream f1;
	f1.open("sim3File.txt");
	ifstream f2;
 	f2.open("currentFrame.txt");
	...
	cout << "number of bad data " << badData << endl;
	if (badData > 4) {
